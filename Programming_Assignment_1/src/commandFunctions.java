...
public class commandFunctions {
    ...
    public static void reRunLastNCommands(String cmd, List<String> cmdHistory) throws IOException {
        ...
        // Start from the most recent command and go backwards
        for (int i = cmdHistory.size() - 1; i >= cmdHistory.size() - n; i--) {
            System.out.println("Executing command: " + cmdHistory.get(i));
            executeCommandFromHistory(cmdHistory.get(i));
            System.out.println();
        }
    }
    ...
    private static void executeCommandFromHistory(String command) throws IOException {
        ...
        switch (cmd) {
            ...
            case "cd" -> commandFunctions.cd(command);
            case "cat" -> commandFunctions.cat(command);
